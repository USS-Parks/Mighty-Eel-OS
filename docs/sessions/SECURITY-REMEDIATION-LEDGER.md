...
| AF-11 | Restore manifest component paths can escape backup/target roots | Medium | PSPR-23 | `tools/mai-admin/src/restore.rs` (`validate_component_path`) | 29, 31 | `component_path_validation_rejects_escape` | `M3/F5` | validated relative path used for both read and write | closed | CODE-FIXED |
...
| AF-15B | AOG revocation check fails open when snapshot is absent | Medium | PSPR-07 | `crates/aog-gateway/src/lib.rs` (`revocation_decision`) | 28, 31 | gateway revocation unit tests (stale / fail-closed / revoked-tenant / clean) | `M3/F2` | configured path + absent snapshot fails closed; complete predicate + freshness | closed; N1 all-dimensions + N3 freshness folded in | CODE-FIXED |
...
| AF-19 | Restore accepts unsigned/unverified manifests by default | Medium | PSPR-22 | `tools/mai-admin/src/main.rs` | 29, 31 | `plan_require_signed_rejects_unsigned` (default-on) | `M3/F5` | verify on by default; `--allow-unsigned` opt-out | closed | CODE-FIXED |
| AF-20 | Production-like deployment images use mutable tags | Medium | PSPR-25 | `deployment/wsf-ha/docker-compose.yml` | 30, 31 | YAML parse + no-`latest` scan | `M3/F8` | first-party image pinned to `${WSF_API_VERSION:-v0.1.0}` | closed; demo-stack floats (N1) dispositioned Low | CODE-FIXED |
| DF-01A | Model package signature authenticates weights but not manifest identity | High | PSPR-17 | `mai-core/src/models/verify.rs` | 29, 31 | `mai-core` manifest-auth tests (authenticated / binding-mismatch / strict-reject) | `M3/F5` | v2 = `manifest.mldsa` + weights-bound `integrity_hash_tree`; strict mode denies legacy | consumer closed; **production ship profile enforces strict manifest at startup** (`mai-api/src/server.rs`); offline signer must emit `manifest.mldsa` | CODE-FIXED |
