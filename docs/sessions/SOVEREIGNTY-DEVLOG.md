...
- **0.7** — CI upgrade (live OpenBao service container, LocalStack, workspace tests on push, tsc/vitest, cargo-audit/deny, hadolint, SBOM) — the only Phase-0 item left before F1.

**Phase 0 status:** 0.1, 0.2a–d, 0.3–0.6, 0.8 DONE + tag `contracts-v1` (10 commits). Only **0.7 (CI)** remains before F1.
