...
> **SUPERSEDED — retained for history (annotated 2026-07-12).** This roadmap is the endpoint of the pre-June-2026 MAI / Lamprey build generation (RC1/RC2 cadence, DOUGHERTY, GitDoctor/IGD scan lanes). That line was revived and folded into the modern **Agentic Orchestration Governance (AOG)** + **Woven Security Fabric (WSF)** stack; the RC2 rehearsal and in-flight scan lanes named below did not continue as planned. Do not read it as the current ship plan — live status lives in the X4/X5 revalidation (host-surface GO) and the repo docs index.
